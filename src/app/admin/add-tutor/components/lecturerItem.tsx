    ...
    <div>
      <ul className="flex gap-32 items-center justify-between w-[500px] bg-[#F3F3F3] rounded-buttonBorder px-6 py-2">
        <div className="flex gap-4 items-center">
          {lecture?.image ? (
            <img
              src={`https://smarteducation.shop/smarteducation_backend/public/${lecture?.image}`}
              alt="lecturer_image"
              className="p-1 w-14 h-14 rounded-[100px]"
            />
          ) : (
            <LoadingSpinner />
          )}
          <div className="flex gap-2 w-full">
            <li className="text-black text-xl font-normal">
              {lecture?.first_name || <LoadingSpinner />}
            </li>
            <li className="text-black text-xl font-normal">
              {lecture?.last_name || <LoadingSpinner />}
            </li>
          </div>
        </div>
